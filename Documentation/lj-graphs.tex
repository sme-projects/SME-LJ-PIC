...
\begin{figure}
    ...
    \scalebox{0.8}{
        \begin{tikzpicture}
            ...
            \node[ram] (veloram) at (5.5,-2) {Velocity RAM};
            \node[simulation] (testsim) at (0,0) {Testing Simulator};
            \node[process] (manager) at (0,-4) {Velocity Manager};
            \node[module] (velocity) at (0,-6.5) {Velocity};
            \node[multiplexer, shape border rotate=270] (mux) at (3.3,-2) {};

            \path[draw, ->] (testsim.250) -- (manager.108) node [near end, left] {\footnotesize simulation ready};
            \path[draw, ->] (testsim.282) -- (manager.80) node [near end] {\footnotesize data ready};
            \path[draw, ->] (velocity.128) -- (manager.222) node [midway] {\footnotesize $V_i$};
            \path[draw, ->] (manager.269) -- (velocity.90) node [midway, left] {\footnotesize $A_i$};
            \path[draw, ->] (manager.297) -- (velocity.70) node [midway] {\footnotesize $V_{i-1}$};

            \path[draw, ->] (manager.350) -| node [near start] {\footnotesize i} (veloram.260) node [very near end] {\footnotesize \texttt{A}};
            \path[draw, ->, red] (veloram.280) |- node [pos=0.11] {\footnotesize \texttt{A}} (manager.340) node [near end] {\footnotesize $V_{i-1}$};

            \path[draw, ->] (manager.180) -| node [near start, above] {\footnotesize i} (accram.280) node [very near end, right] {\footnotesize \texttt{B}};
            \path[draw, ->, red] (accram.260) |- node [pos=0.11, left] {\footnotesize \texttt{B}} (manager.190) node [near end, below] {\footnotesize $A_i$};
            \path[draw, ->] (manager.0) -| (2,-3) |- (mux.south west) node [near end, below] {\footnotesize (2.) $V_i$};
            \path[draw, ->] (testsim.0) -| (2,-1) |- (mux.north west) node [midway, below] {\footnotesize (1.) $V_{init}$/i};
            ...
            \path[draw, ->] (testsim.100) |- (-5, 1) node [near end, above] {\footnotesize $A_{init}$} -| (accram.90) node [very near end] {\footnotesize \texttt{A}};
              ...
              \path[draw, ->, red] (veloram.90) |- node [pos=0.11] {\footnotesize \texttt{B}} (testsim.10) node [near end] {\footnotesize $V_{i}$};
